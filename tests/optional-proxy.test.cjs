...
test('optional video proxies preserve original metadata, persist preferences and decode smaller frames',async()=>{
 ...
 const file=path.join(dir,'原本.mp4');await run(ffmpeg,['-y','-v','error','-f','lavfi','-i','testsrc2=size=1920x1080:rate=30:duration=1','-c:v','libx264','-preset','ultrafast','-pix_fmt','yuv420p',file]);
