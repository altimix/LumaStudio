...
const {DEFAULT_GAUSSIAN_BLUR}=require('../shared/gaussian-blur.mjs');
let dir,asset;
before(async()=>{
...
function project(mosaic,gaussianBlur){
  const clip={id:'c',assetId:asset.id,trackId:'v',kind:'image',name:'模様',start:0,in:0,duration:1,speed:1,x:0,y:0,scale:1,rotation:0,opacity:1,exposure:0,contrast:1,saturation:1,volume:1,fadeIn:0,fadeOut:0,text:'',fontSize:40,color:'#ffffff',textStyle:'minimal',...(mosaic!==undefined?{mosaic}:{}),...(gaussianBlur!==undefined?{gaussianBlur}:{})};
...
test('softens mosaic cell edges only inside the selected image region in MP4',async()=>{
  ...
  const boundary=32+Math.round(128*mosaic.blockSize),before=gray(result,boundary-5,61),middle=gray(result,boundary,61),after=gray(result,boundary+5,61);
  assert.ok(Math.abs(after-before)>40&&middle>Math.min(before,after)+5&&middle<Math.max(before,after)-5,'mosaic boundary fades between cells');
});

test('mosaic blur never mixes unpixelated pixels across its region edge',async()=>{
  const pixels=Buffer.alloc(128*128);
  for(let y=0;y<128;y++)for(let x=0;x<128;x++)pixels[y*128+x]=x<32?255:0;
  const source=path.join(dir,'edge.pgm'),image=path.join(dir,'edge.png');
  await fs.writeFile(source,Buffer.concat([Buffer.from('P5\n128 128\n255\n'),pixels]));
  await run(ffmpeg,['-v','error','-y','-i',source,'-frames:v','1',image]);
  const edgeAsset=await inspectMedia(image,path.join(dir,'edge-cache'));
  const p=project({x:.5,y:.5,width:.5,height:.5,blockSize:.1});
  p.assets=[edgeAsset];p.clips[0].assetId=edgeAsset.id;
  const output=path.join(dir,'edge-mosaic.mp4');
  await exportProject(p,{width:128,height:128,fps:10,quality:'high',encoder:'cpu'},output);
  const result=await frame(output);
  assert.ok(gray(result,33,64)<15,'inside edge samples only pixelated dark cells');
  assert.ok(gray(result,29,64)>230,'outside edge remains the original bright image');
});

test('Gaussian blur also works on an image independently of mosaic',async()=>{
  const blur={...DEFAULT_GAUSSIAN_BLUR,width:.5,height:.5,sigma:.03};
  const output=path.join(dir,'gaussian-image.mp4'),settings={width:128,height:128,fps:10,quality:'high',encoder:'cpu'};
  await exportProject(project(undefined,blur),settings,output);
  const result=await frame(output);
  assert.ok(Math.abs(gray(result,61,61)-gray(result,65,61))<35,'selected image area is Gaussian blurred');
  assert.ok(Math.abs(gray(result,16,16)-gray(result,20,16))>100,'image detail outside the selection stays sharp');
});
