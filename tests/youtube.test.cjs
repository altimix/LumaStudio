...
test('selected Sol reaches Responses API and other text models are rejected before a request', async () => {
  const calls=[];
  const client=createOpenAI(async()=>KEY,async(_url,options)=>{calls.push(JSON.parse(options.body));return Response.json({status:'completed',output:[{content:[{type:'output_text',text:'{"ok":true}'}]}]});});
  assert.deepEqual(await client.metadata({transcript:'字幕'},undefined,'gpt-6-sol'),{ok:true});
  ...
  await assert.rejects(client.metadata({},undefined,'gpt-6-sol-max'),/投稿文モデル/);
