...
(async()=>{
 ...
 const retainedRecovery=await fs.readFile(path.join(profile,'autosave.luma'),'utf8');
 await dialog.locator('article').filter({hasText:'以前の版'}).getByRole('button',{name:'この版を復元'}).click();await page.getByRole('button',{name:'以前の版',exact:true}).waitFor();assert.equal(await page.locator('.unsaved-dot').count(),1);assert.equal(await page.locator('.recovery-banner').count(),0);assert.equal(await fs.readFile(path.join(profile,'autosave.luma'),'utf8'),retainedRecovery);
