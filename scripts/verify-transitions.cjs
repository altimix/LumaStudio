...
async function verify(){
  ...
  try{
    ...
    const firstUnchanged=await save();assert.deepEqual(firstUnchanged.clips.map(c=>c.start),[0,3]);assert.equal(firstUnchanged.transitions?.length||0,0);assert.ok(await page.getByRole('button',{name:/^元に戻す \(/,exact:true}).isDisabled());checks.push('a single first-clip selection is rejected without moving later clips or creating history');
    ...
    await page.getByRole('button',{name:'音声',exact:true}).click();await page.locator('.transition-list button').filter({hasText:'コンスタントゲイン'}).click();saved=await save();assert.equal(saved.transitions[0].video,'pagePeel');assert.equal(saved.transitions[0].audio,'constantGain');await page.getByRole('button',{name:'切り替え',exact:true}).click();for(const name of ['クロスディゾルブ','ページピール']){await page.locator('.transition-list button').filter({hasText:name}).click();assert.equal((await save()).transitions[0].audio,'constantGain','video-only changes retain the chosen gain curve');}await page.keyboard.press('Control+o');await page.waitForFunction(()=>document.querySelector('button[aria-label^="元に戻す ("]').disabled);await incoming.focus();await page.keyboard.press('Enter');assert.equal((await save()).transitions[0].audio,'constantGain');assert.equal(await page.locator('.timeline-clip.offline').count(),0);assert.equal(await page.locator('.media-card.offline').count(),0);
    const track=p.tracks.find(t=>t.id===v.trackId);await page.getByRole('button',{name:track.name+' ロック',exact:true}).click();assert.ok(await page.getByRole('button',{name:'効果を解除',exact:true}).isDisabled());assert.equal((await save()).transitions.length,1);await page.getByRole('button',{name:track.name+' ロック解除',exact:true}).click();await page.getByRole('button',{name:'効果を解除',exact:true}).click();saved=await save();assert.equal(saved.transitions.length,1);assert.equal(saved.transitions[0].video,undefined);assert.equal(saved.transitions[0].audio,'constantGain');await page.getByRole('button',{name:'音声',exact:true}).click();await page.getByRole('button',{name:'効果を解除',exact:true}).click();saved=await save();assert.equal(saved.transitions.length,0);assert.equal(saved.clips[1].start,3);checks.push('audio curve changes retain video, saved transitions reload, locks protect removal, removal retains placement');
    ...
    await page.getByRole('button',{name:'切り替え',exact:true}).click();await page.getByLabel('トランジションの長さ',{exact:true}).fill('5');await page.getByRole('button',{name:'クロスディゾルブ 2つの映像をなめらかに重ねる',exact:true}).click();
    await page.getByText('間に別のクリップがあります。隣り合う2つのクリップを選んでください。',{exact:true}).waitFor();
    const unchanged=await save();assert.deepEqual(unchanged.clips.map(c=>c.start),[0,6,8]);assert.equal(unchanged.transitions.length,0);assert.ok(await page.getByRole('button',{name:/^元に戻す \(/,exact:true}).isDisabled());checks.push('nonadjacent selected clips are rejected without changing the original order or editing history');
