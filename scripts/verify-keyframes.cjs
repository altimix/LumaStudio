...
async function verify(){
  ...
  try{
    ...
    await page.getByRole('group',{name:'プロパティの表示項目',exact:true}).getByRole('button',{name:'カラー',exact:true}).click();assert.deepEqual(await highlighted(),[]);assert.match(await page.locator('.visual-channel-help').innerText(),/「ビデオ」タブ/);assert.doesNotMatch(await page.locator('.visual-channel-help').innerText(),/黄緑/);
