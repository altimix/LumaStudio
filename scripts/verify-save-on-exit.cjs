...
(async()=>{
  ...
  const finish=async choice=>{const closed=app.waitForEvent('close',{timeout:30000});await requestClose(choice);await closed;app=null;};
  ...
  try{
    await launch();await edit('新規の保存前');
    ...
    await app.evaluate(()=>{globalThis.__restoreRename();globalThis.__releaseWrite();});await page.locator('.toast').filter({hasText:'保存中に新しい変更'}).waitFor();assert.ok(!page.isClosed());assert.ok((await saved()).clips.some(c=>c.name==='保存開始時点'));assert.equal(await page.getByRole('textbox',{name:'クリップ名',exact:true}).inputValue(),'保存中に到着した変更');await page.locator('.unsaved-dot').waitFor();await finish(2);assert.ok((await saved()).clips.some(c=>c.name==='保存中に到着した変更'));
