...
(async()=>{
  ...
  try{
    ...
    await page.evaluate(()=>{window.luma.onSaveBeforeClose(id=>{window.__closeRequest=id;});});await remain(2);const savingDialog=page.getByRole('dialog',{name:'プロジェクトを保存しています',exact:true});await savingDialog.waitFor();assert.equal(await savingDialog.locator('.project-operation-status').count(),1);assert.equal(await savingDialog.getByRole('status').getAttribute('aria-live'),'polite');
