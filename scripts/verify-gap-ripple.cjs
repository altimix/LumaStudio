...
async function verify() {
  ...
  try {
    ...
    const save=async()=>{const before=(await fs.stat(file)).mtimeMs;await page.getByRole('button',{name:/^プロジェクトを保存 \(/,exact:true}).click();await page.waitForFunction(()=>!document.querySelector('.unsaved-dot'));const deadline=Date.now()+10000;while((await fs.stat(file)).mtimeMs===before){assert.ok(Date.now()<deadline);await new Promise(r=>setTimeout(r,25));}return JSON.parse(await fs.readFile(file,'utf8'));};
    const open=async p=>{await save();await fs.writeFile(file,JSON.stringify(p));await page.keyboard.press('Control+o');await page.getByRole('button',{name:p.name,exact:true}).waitFor();await page.waitForFunction(()=>document.querySelector('button[aria-label^="元に戻す ("]').disabled);};
    ...
    await page.keyboard.press('Control+o');await page.waitForFunction(()=>document.querySelector('button[aria-label^="元に戻す ("]').disabled);edited=await save();assert.equal(await clip('after').getAttribute('aria-label'),'後ろの映像、開始 2.00 秒、長さ 2.00 秒');
