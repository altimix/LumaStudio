 ...
 try{
  ...
  for(const scenario of ['landscape','portrait','text-only']){
   ...
   if(portrait){
    ...
    await app.evaluate(({dialog},file)=>{dialog.showOpenDialog=async()=>({canceled:false,filePaths:[file]});},file);await page.keyboard.press('Control+o');await page.getByRole('button',{name:reloaded.name,exact:true}).waitFor();await page.getByRole('button',{name:'YouTube',exact:true}).click();await page.getByRole('button',{name:'サムネイル',exact:true}).click();await page.getByAltText('サムネイルに取り込む参考画像',{exact:true}).waitFor();assert.equal(await page.locator('.yt-reference-preview span').innerText(),path.basename(replacement));
   }
  ...
  await fs.writeFile(path.join(results,'requests.json'),JSON.stringify(calls,null,2));console.log('GPT Image 2.5: optional reference selection/replacement/removal, Undo/Redo/reload, unregistered file rejection, first-image fidelity, no-reference generation, both orientations, JPEG under 2 MB, and failure retention verified (API mocked).');
