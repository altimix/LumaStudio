...
async function verify(){
 ...
 try{
  ...
  await page.getByRole('button',{name:'更新を確認',exact:true}).click();await page.getByText('新しい安定版はありません。',{exact:false}).waitFor();await page.screenshot({path:path.join(results,'help-updates-current.png')});
  assert.equal(await page.getByRole('button',{name:'ダウンロードページを開く'}).count(),0);assert.equal(await page.locator('.update-steps').count(),0);
  ...
  await page.getByRole('button',{name:'更新を確認',exact:true}).click();await page.getByText('Luma Studio 2.0.0 が利用できます。',{exact:false}).waitFor();await page.getByRole('button',{name:'ダウンロードページを開く'}).waitFor();assert.equal(await page.locator('.update-steps').count(),1);await page.screenshot({path:path.join(results,'help-updates.png')});await page.keyboard.press('Escape');await page.getByRole('button',{name:'更新あり',exact:true}).click();await page.getByRole('dialog',{name:'アップデート',exact:true}).waitFor();
  await app.evaluate(({ipcMain,app})=>{ipcMain.removeHandler('check-updates');ipcMain.handle('check-updates',()=>({status:'unsupported',currentVersion:app.getVersion(),latestVersion:'2.0.0',checkedAt:Date.now(),platform:'darwin',arch:'x64'}));});
  await page.getByRole('button',{name:'更新を確認',exact:true}).click();await page.getByText('異なるOS・アーキテクチャ向けのアプリに入れ替えず、',{exact:false}).waitFor();assert.equal(await page.locator('.update-steps').count(),0);assert.equal(await page.getByText('macOS-arm64.zip',{exact:false}).count(),0);
