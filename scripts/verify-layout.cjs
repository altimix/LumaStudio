...
(async () => {
  ...
  try {
    ...
    await button('元に戻す (Ctrl+Z)').click(); assert.deepEqual(await save(), withAudio);
    await button('やり直す (Ctrl+Shift+Z)').click(); assert.deepEqual(await save(), withBgm);
    await page.locator('.timeline-clip.image').click();
    ...
    await button('元に戻す (Ctrl+Z)').click(); assert.deepEqual(await save(), withBgm);
    await button('やり直す (Ctrl+Shift+Z)').click(); assert.deepEqual(await save(), withDrawing);
