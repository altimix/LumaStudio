...
(async () => {
  ...
  try {
    ...
    const customFps={...beforePreset,name:'48fpsの書き出し設定',fps:48,clips:[beforePreset.clips.find(clip=>clip.kind==='image')],transitions:[]};
    await fs.writeFile(projectFile,JSON.stringify(customFps));await openProject();await button(customFps.name).waitFor();await button('書き出し').click();
    const fps=page.getByLabel('書き出しフレームレート',{exact:true});assert.equal(await fps.inputValue(),'48');await fps.selectOption('30');await fps.selectOption('48');assert.equal(await fps.inputValue(),'48');await page.keyboard.press('Escape');
    checks.push('custom sequence FPS is displayed and can be reselected in export');
