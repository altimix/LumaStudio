...
バージョンはルートの`package.json`から読み取ります。`build.mjs`の更新内容・配布ファイル名・対応条件とREADME.htmlのマニュアルを新しい公開版に合わせます。まだ配布していない機能を提供済みとして掲載しません。公開前後にリンクと表示を検証します。
...
トップの`assets/workspace.png`は1.5.0の実アプリの編集画面です。1.6.0でも基本配置を継承していることを明記し、画像そのものを1.6.0の画面とは表示しません。青い背景と文字を使った操作例で、元画像の加工や架空UIの生成はしていません。従来の`assets/editor.png`とその映像クレジットも保持しています。
