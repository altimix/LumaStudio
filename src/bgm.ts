...
export function insertBgm(p: Project, asset: Asset, at: number, fit: boolean, volume: number) {
  ...
  let tracks = p.tracks;
  let track = tracks.find(t => t.kind === 'audio' && !t.locked && !t.muted && !t.hidden && !p.clips.some(c => c.trackId === t.id && c.start < end-1e-7 && c.start+c.duration > start+1e-7));
  if (!track) { if (tracks.length >= 24) throw new Error('BGMを置く空きトラックがありません。音楽トラックの空き区間を選ぶか、トラックを減らしてください。'); track = makeTrack('audio'); tracks = [...tracks,track]; }
  ...
  return { project:numberTracks({...base,clips:[...p.clips,...added]}), ids:added.map(c=>c.id), repeats:count };
}
