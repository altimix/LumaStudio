...
export default function ShortcutHelp({ onClose }: { onClose(): void }) {
  ...
  return <Modal title="キーボードショートカット" onClose={onClose} wide>
    ...
    <KeyboardDiagram mac={mac} setMac={setMac}/>
    ...
    <div className="shortcut-list">{matches.map(([key, action]) => <div key={key}><span>{action}</span><kbd>{displayKey(key)}</kbd></div>)}{!matches.length ? <p>一致するショートカットがありません。</p> : null}</div>
