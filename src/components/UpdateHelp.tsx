...
export default function UpdateHelp({info,checking,onCheck}:{info:UpdateInfo|null;checking:boolean;onCheck():void}) {
 ...
 const supported=!!info && info.status!=='unsupported' && ((mac&&info.arch==='arm64')||(info.platform==='win32'&&info.arch==='x64'));
 ...
 <h3>新しいバージョンへの入れ替え</h3>{supported?<ol className="update-steps"><li>編集中のプロジェクトを保存し、書き出しが終わるのを待ちます。</li><li>ダウンロードページで{mac?'macOS-arm64.zip':'Windows.exe'}を取得します。必要に応じてSHA256SUMS.txtで確認できます。</li><li>Luma Studioを終了します。</li><li>{mac?'ZIPを展開し、今までのLuma Studio.appを新しいアプリに入れ替えます。':'新しいEXEを保存して起動します。古いEXEは新しい版の動作確認後に削除できます。'}</li><li>新しいアプリから保存済みのプロジェクトを開きます。プロジェクトや元の素材ファイルは削除しないでください。</li></ol>:<p className="field-help">この環境に対応する配布ファイルを確認できていません。異なるOS・アーキテクチャ向けのアプリに入れ替えず、配布ページで対応状況を確認してください。</p>}
 {supported&&mac&&<p className="field-help">未署名のMac版がブロックされた場合は、システム設定 → プライバシーとセキュリティから、このアプリの起動を許可してください。詳しくは配布ページのREADME-Mac.txtをご覧ください。</p>}
