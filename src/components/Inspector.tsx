...
function EffectField({ clip, label, value, min, max, step = 1, suffix = '', patch }: { clip: Clip; label: string; value: number; min: number; max: number; step?: number; suffix?: string; patch: (clip: Clip, value: number) => Partial<Clip> }) {
  const [draft,setDraft]=useState(String(Number(value.toFixed(2)))),[focused,setFocused]=useState(false),edited=useRef(false),dragging=useRef(false),changed=useRef(false);
  ...
  const finish=(cancel=false)=>{
    ...
    if(changed.current&&(cancel||(oldClip&&nowClip&&JSON.stringify(oldClip)===JSON.stringify(nowClip))))useEditor.setState({project:before.project,history:before.history,future:before.future,historyPlayheads:before.historyPlayheads,futurePlayheads:before.futurePlayheads,historyLabels:before.historyLabels,futureLabels:before.futureLabels,currentAction:before.currentAction,dirty:before.dirty});
    current.endGesture(start.owner);changed.current=false;
  };
  ...
  const apply=(input:number)=>{
    ...
    if(dragging.current){if(!changed.current){state.checkpoint(`${label}を変更`);changed.current=true;}const now=useEditor.getState();now.transient({...now.project,clips:now.project.clips.map(c=>c.id===current.id?normalizeClip({...c,...values},now.project):c)});}
    else state.updateClip(current.id,values);
  };
  return <div className="property-field"><div className="property-label"><label htmlFor={inputId}>{label}</label><div className="number-wrap"><input id={inputId} type="number" min={min} max={max} step={step} value={draft} onFocus={()=>{setFocused(true);edited.current=false;}} onChange={e=>{setDraft(e.target.value);edited.current=true;}} onBlur={()=>{setFocused(false);const wasEdited=edited.current;edited.current=false;if(wasEdited&&draft!==''&&Number(draft)!==value)apply(Number(draft));else setDraft(String(Number(value.toFixed(2))));}} onKeyDown={e=>{if(e.key==='Enter')e.currentTarget.blur();}}/><span>{suffix}</span></div></div><input className="property-slider" type="range" aria-label={`${label}スライダー`} min={min} max={max} step={step} value={value} style={{'--fill':`${max>min?(value-min)/(max-min)*100:100}%`} as React.CSSProperties} onPointerDown={e=>{if(e.button!==0)return;const state=useEditor.getState(),owner={};if(state.gestureActive||!state.beginGesture(owner,()=>finish(true)))return;dragStart.current={state,owner};dragging.current=true;changed.current=false;}} onPointerUp={()=>finish()} onPointerCancel={()=>finish(true)} onLostPointerCapture={()=>finish(true)} onChange={e=>apply(Number(e.target.value))}/></div>;
}
