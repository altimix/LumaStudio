...
export default function KeyboardDiagram({mac,setMac}:{mac:boolean;setMac(value:boolean):void}) {
 ...
 <div className="keyboard-scroll"><div className="keyboard-keys">{rows.map((row,i)=><div className="keyboard-row" key={i}>{row.map(key=><button key={key} className={`${command(key)?'assigned':''} ${key===' '?'space-key':''}`} aria-pressed={selected===key} aria-label={`${display(key)}：${command(key)?labels[command(key)!]:'割り当てなし'}`} onClick={()=>setSelected(key)}><kbd>{display(key)}</kbd>{command(key)&&<span>{labels[command(key)!]}</span>}</button>)}</div>)}</div></div>
