...
export default function MediaDragLayer({ sizes, actions, onSampleChroma }: { sizes: Record<string, MediaSize>; actions: HTMLElement | null; onSampleChroma:(clipId:string,point:{x:number;y:number})=>string|Promise<string> }) {
  ...
  const startEffect = (event:ReactPointerEvent<HTMLButtonElement>,renderedClip:Clip,source:SourceSize,operation:EffectOperation) => {
    ...
    const action=operation.kind==='crop'?'クロップ範囲を変更':operation.kind==='mosaic'?'モザイクぼかし範囲を変更':operation.kind==='gaussianBlur'?'ガウスぼかしの範囲を変更':'マスクを変更';
  ...
  const effectOverlay=()=>{
    ...
    if(editMode==='mosaic'&&clip.mosaic){const mosaic=clip.mosaic,center=mediaPoint(clip,source,project,{x:mosaic.x,y:mosaic.y});return <><button className="media-effect-box mosaic" aria-label="モザイクぼかし範囲を移動" style={style(center,bounds.width*mosaic.width,bounds.height*mosaic.height)} onPointerDown={e=>startEffect(e,clip,source,{kind:'mosaic'})}/>{corners.map(corner=>{const point=mediaPoint(clip,source,project,{x:mosaic.x+corner.x*mosaic.width/2,y:mosaic.y+corner.y*mosaic.height/2});return <button key={corner.name} className="media-effect-handle mosaic" aria-label={`モザイクぼかし範囲の${corner.name}を変更`} style={{left:point.x/project.width*100+'%',top:point.y/project.height*100+'%',cursor:corner.cursor,zIndex:z+1}} onPointerDown={e=>startEffect(e,clip,source,{kind:'mosaic',corner})}/>;})}</>}
