...
export default function EditingGuide({ onImport, onTitles, onExport, onNew, onShortcuts, hasClips }: { onImport():void; onTitles():void; onExport():void; onNew():void; onShortcuts():void; hasClips:boolean }) {
  return <div className="editing-guide"><p className="guide-intro">まずは短い動画で試してみましょう。操作を間違えても、<kbd>{shortcutLabel('Ctrl（Macは⌘）')}</kbd> + <kbd>Z</kbd> で戻せます。</p>
    <ol className="guide-steps">
      ...
      <li><span className="guide-number">3</span><div><h3>いらない部分を切り、文字や音を整える</h3><p><kbd>Space</kbd> で再生・停止。切りたいクリップを選び、切りたい位置に再生ヘッドを動かして <kbd>{shortcutLabel('Ctrl（Macは⌘）')}</kbd> + <kbd>B</kbd> で分割します。不要な部分を選んで <kbd>{shortcutLabel('Delete')}</kbd> で削除できます。</p><p>文字はプレビューでドラッグできます。会話の音量は、クリップを選んで「オーディオ」から自動調整できます。</p><button className="secondary-button" onClick={onTitles}><Type size={15}/>文字を追加する</button></div></li>
      <li><span className="guide-number">4</span><div><h3>保存して、書き出す</h3><p><kbd>{shortcutLabel('Ctrl（Macは⌘）')}</kbd> + <kbd>S</kbd> は、後で編集を続けるための保存です。「書き出し」で動画のMP4か音声のみのMP3を選べます。MP4は「自動」のまま利用可能なGPUを使えます。</p><button className="primary-button" onClick={onExport} disabled={!hasClips}><Download size={15}/>書き出しへ</button></div></li>
