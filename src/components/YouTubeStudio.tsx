...
export default function YouTubeStudio({ onClose }: { onClose: () => void }) {
  ...
  const accept = (snapshot: Project, result: YoutubeData, assets?: Project['assets']) => {
    ...
    if (current.id !== snapshot.id || timelineKey(current) !== timelineKey(snapshot)) throw new Error('処理中にタイムラインが変わりました。現在の内容で再実行してください。');
  ...
  return <Modal title="YouTube制作スタジオ" wide onClose={() => { if (!busy) onClose(); }}><div className="youtube-studio">
    ...
    <div className="yt-scroll" ref={scroll}><fieldset className="yt-content" disabled={busy} onBlurCapture={() => { editingField.current = ''; }}>
      ...
      {tab === 'thumbnail' ? <div className="yt-thumbnail"><div><h3>ImageGenでサムネイルを作成</h3><p className="yt-hint">{p.height > p.width ? '864 × 1536 / 9:16' : '1536 × 864 / 16:9'} · JPEG。投稿文の生成後は、提案されたプロンプトを編集して使えます。</p><label>画像の生成指示<textarea aria-label="サムネイルの生成指示" rows={8} maxLength={6000} value={y.thumbnailPrompt} onChange={e => update({ thumbnailPrompt: e.target.value }, 'thumbnail-prompt')}/></label><button className="primary-button" disabled={!status?.configured || !y.thumbnailPrompt.trim()} onClick={() => void task(async current => { const asset = await window.luma!.aiThumbnail(current, current.youtube!.thumbnailPrompt); accept(current, { ...current.youtube!, thumbnailAssetId: asset.id }, [...current.assets, asset]); })}><ImagePlus size={15}/>サムネイルを1枚生成</button><p className="yt-hint">生成後に文字や内容を確認してください。再生成ではAPI利用料が追加で発生します。</p></div><div className="yt-thumbnail-preview">{thumbnail ? <><img src={thumbnail.url} alt="生成したYouTubeサムネイル"/><button className="secondary-button" onClick={() => void save('jpg')}><Download size={15}/>JPEGを保存</button></> : <div className="yt-empty"><ImagePlus size={42}/><span>生成した画像をここで確認</span></div>}</div></div> : null}
    </fieldset></div>
