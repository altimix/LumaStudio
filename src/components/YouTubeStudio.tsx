...
import { DEFAULT_TEXT_MODEL, TEXT_MODELS, validateTextModel, type TextModel } from '../../shared/ai-text-model.mjs';
...
const TEXT_MODEL_STORAGE_KEY = 'luma.youtube.text-model.v1';
function preferredTextModel(): TextModel {
  try { return validateTextModel(window.localStorage.getItem(TEXT_MODEL_STORAGE_KEY) ?? undefined); }
...
export default function YouTubeStudio({ onClose }: { onClose: () => void }) {
  ...
  const [textModel, setTextModel] = useState<TextModel>(preferredTextModel);
  ...
  const [draftChapters, setDraftChapters] = useState<YoutubeData['chapters']>();
  const selectTextModel = (value: string) => {
    const model = validateTextModel(value);
    setTextModel(model);
    try { window.localStorage.setItem(TEXT_MODEL_STORAGE_KEY, model); } catch { /* The current selection remains usable. */ }
  };
  const previewChapters = draftChapters ?? y.chapters;
  const draftCommitFailed = useRef(false);
  ...
  const update = (patch: Partial<YoutubeData>) => {
    setCopied('');
    try {
      ...
      s.commit({ ...s.project, youtube: next }, 'YouTube投稿素材を編集');
  ...
  const accept = (snapshot: Project, result: YoutubeData, assets?: Project['assets'], action = 'YouTube制作素材を生成') => {
  ...
  const cueDraft = (index: number, patch: Partial<SubtitleCue>) => ({ cues: y.cues.slice(Math.max(0, index - 1), index + 2).map(c => c === y.cues[index] ? { ...c, ...patch } : c) });
  const checkDraft = (patch: Partial<YoutubeData>) => {
    draftCommitFailed.current = false;
    if (patch.chapters) setDraftChapters(patch.chapters);
    setCopied('');
    try { validateYoutube({ ...y, cues: [], ...patch }); invalidDraft.current = false; setDraftInvalid(false); setError(''); }
  ...
  const commitDraft = (patch: Partial<YoutubeData>, input: HTMLInputElement | HTMLTextAreaElement, saved: string) => {
    draftCommitFailed.current = !update(patch);
    if (draftCommitFailed.current) input.value = saved;
    if (patch.chapters) setDraftChapters(undefined);
  ...
  const close = () => {
    ...
    // Escape/backdrop unmounts before the browser's normal blur. Flush while
    // the input and its React handler still exist, and keep errors visible.
    if (active instanceof HTMLElement && scroll.current?.contains(active)) active.blur();
    if (draftCommitFailed.current) { draftCommitFailed.current = false; return false; }
    useEditor.getState().stop(); onClose(); return true;
  };
  const filtered = useMemo(() => y.cues.map((cue, index) => ({ cue, index })).filter(({ cue }) => !search || cue.text.includes(search)), [y.cues, search]);
  const rows = filtered.slice(page * 40, page * 40 + 40);
  const thumbnail = p.assets.find(a => a.id === y.thumbnailAssetId);
  const reference = p.assets.find(a => a.id === y.thumbnailReferenceAssetId);
  const referenceMissing = !!y.thumbnailReferenceAssetId && (!reference || reference.offline || reference.kind !== 'image');
  ...
  return <Modal title="YouTube制作スタジオ" wide onClose={close}><div className="youtube-studio">
    ...
    <div className={`yt-workspace ${tab === 'captions' ? 'with-preview' : ''}`}>{tab === 'captions' ? <CaptionMonitor cues={y.cues} index={Math.min(activeCue, Math.max(0, y.cues.length - 1))} disabled={busy || stale || draftInvalid} onSelect={selectCue}/> : null}<div className="yt-scroll" ref={scroll}><fieldset className="yt-content" disabled={busy}>
      {tab === 'settings' ? <div className="yt-settings"><h3>OpenAI APIの設定</h3><p>文字起こし時に音声を、投稿文・画像生成時に入力内容をOpenAIへ送信します。実行した分のAPI利用料がかかります。</p><label>OpenAI APIキー<input aria-label="OpenAI APIキー" type="password" autoComplete="new-password" placeholder="sk-…" value={key} onChange={e => setKey(e.target.value)}/></label><div className="yt-actions"><button className="primary-button" disabled={!key || !window.luma} onClick={() => void task(async () => { const value = key; setKey(''); const s = await window.luma!.aiSetKey(value); if (alive.current) setStatus(s); })}>キーを保存</button><button className="secondary-button" disabled={!window.luma} onClick={() => void task(async () => { const s = await window.luma!.aiImportEnv(); if (alive.current) setStatus(s); })}>.envから読み込む</button><button className="text-button" disabled={!status?.configured} onClick={() => void task(async () => { const s = await window.luma!.aiClearKey(); if (alive.current) setStatus(s); })}>保存したキーを解除</button></div><p>{status?.configured ? `利用元: ${status.source}` : 'APIキーを入力するか、OPENAI_API_KEYがある.envを選択してください。'} キーはこのWindowsユーザー用に暗号化して保存します。</p><p>文字起こしは gpt-transcribe、字幕時刻の取得は whisper-1 を使用します。発話のある区間では両モデルのAPI利用料がかかります。時刻が合わない区間の自動再試行にもAPI利用料がかかります。</p><div className="yt-models"><span>日本語の文字起こし<strong>{status?.transcriptionModel || 'gpt-transcribe'}</strong></span><span>投稿文<strong>{textModel}</strong></span><span>画像生成<strong>{status?.imageModel || 'gpt-image-2.5-sunburst'}</strong></span></div></div> : null}
      {tab === 'captions' ? <><div className="yt-toolbar"><label>固有名詞・専門用語のヒント<input aria-label="日本語の用語ヒント" maxLength={120} placeholder="例：Luma Studio、動画編集、リップルトリミング" value={vocabulary} onChange={e => setVocabulary(e.target.value)}/></label><button className="primary-button" disabled={!status?.configured || !p.clips.length} onClick={() => void task(async current => { const result = await window.luma!.aiTranscribe(current, vocabulary); accept(current, result); setPage(0); setActiveCue(0); })}><Sparkles size={15}/>{y.cues.length ? '文字起こしを再実行' : '日本語で文字起こし'}</button></div><p className="yt-hint">長い音声も自動で区切り、本文と時刻が合わない区間だけ再確認します。短い区間でも一致しない場合は、時刻付き認識の本文を採用します。字幕は20〜30文字を目安に句読点・発話の間で区切ります。白文字・青い縁取り・影なしで、横動画とショートに合わせた大きさを使います。編集済みの音声を使用します。会話以外のトラックをミュートすると認識しやすくなります。再実行すると現在の字幕原稿と投稿文を置き換えます。</p>
      {y.cues.length ? <><div className="yt-search"><input aria-label="字幕内を検索" placeholder="字幕内を検索" value={search} onChange={e => { setSearch(e.target.value); setPage(0); }}/><input aria-label="字幕の置換後の文字" placeholder="置換後の文字" value={replacement} onChange={e => setReplacement(e.target.value)}/><button className="secondary-button" disabled={!search} onClick={() => update({ cues: y.cues.map(c => ({ ...c, text: c.text.replaceAll(search, replacement) })) })}>すべて置換</button></div><div className="yt-cue-list">{rows.map(({ cue, index }) => <div className={`yt-cue ${activeCue === index ? 'active-cue' : ''}`} key={index}><button className="yt-cue-jump" title="この字幕の映像を確認" aria-label={`字幕${index + 1}の映像を確認`} aria-pressed={activeCue === index} onClick={() => selectCue(index)}>{String(index + 1).padStart(3, '0')}</button><div className="yt-cue-times"><label>開始秒<input aria-label={`字幕${index + 1}の開始秒`} type="number" min={0} step={0.01} key={cue.start} defaultValue={Number(cue.start.toFixed(3))} onChange={e => checkDraft(cueDraft(index, { start: seconds(e.target.value) }))} onBlur={e => commitDraft(cuePatch(index, { start: seconds(e.target.value) }), e.currentTarget, String(Number(cue.start.toFixed(3))))}/></label><label>終了秒<input aria-label={`字幕${index + 1}の終了秒`} type="number" min={0} step={0.01} key={cue.end} defaultValue={Number(cue.end.toFixed(3))} onChange={e => checkDraft(cueDraft(index, { end: seconds(e.target.value) }))} onBlur={e => commitDraft(cuePatch(index, { end: seconds(e.target.value) }), e.currentTarget, String(Number(cue.end.toFixed(3))))}/></label></div><textarea aria-label={`字幕${index + 1}の本文`} rows={2} maxLength={4000} key={cue.text} defaultValue={cue.text} onChange={e => checkDraft(cueDraft(index, { text: e.target.value }))} onBlur={e => commitDraft(cuePatch(index, { text: e.target.value }), e.currentTarget, cue.text)}/></div>)}</div><div className="yt-pagination"><button disabled={!page} onClick={() => setPage(page - 1)}>前の40件</button><span>{Math.min(page * 40 + 1, filtered.length)}–{Math.min((page + 1) * 40, filtered.length)} / {filtered.length}件</span><button disabled={(page + 1) * 40 >= filtered.length} onClick={() => setPage(page + 1)}>次の40件</button></div><div className="yt-actions"><button className="primary-button" disabled={stale || draftInvalid} onClick={() => { if (invalidDraft.current) return; try { const s = useEditor.getState(); const next=applySubtitles(s.project); validateYoutubeProject(next); if(!s.commit(next, '日本語字幕をタイムラインに適用'))return; s.notify('字幕を適用しました。既存の自動字幕は更新され、Undoで戻せます。'); } catch (e) { setError(message(e)); } }}><Captions size={15}/>字幕をタイムラインに適用</button><button className="secondary-button" disabled={draftInvalid} onClick={() => void save('srt')}><Download size={14}/>SRT保存</button><button className="secondary-button" disabled={draftInvalid} onClick={() => void save('vtt')}>VTT保存</button></div></> : <div className="yt-empty"><Captions size={36}/><strong>話した言葉を、読みやすい日本語字幕に。</strong><p>時刻付きの原稿を作成し、誤変換を直してからタイムラインに適用できます。</p></div>}</> : null}
      {tab === 'metadata' ? <><div className="yt-toolbar yt-metadata-toolbar"><div><h3>動画の内容から、投稿文を作成</h3><p className="yt-hint">タイトル3案・チャプター付き概要欄・ハッシュタグ3個・検索ワード10個</p></div><label className="yt-text-model">投稿文モデル<select aria-label="投稿文モデル" value={textModel} onChange={event => selectTextModel(event.target.value)}>{TEXT_MODELS.map(model => <option key={model} value={model}>{model}</option>)}</select></label><button className="primary-button" disabled={!status?.configured || !y.cues.length || stale} onClick={() => void task(async current => { const result = await window.luma!.aiMetadata(current, textModel); accept(current, result); })}><Sparkles size={15}/>投稿文を生成</button></div>{y.titles.length ? <div className="yt-metadata"><div><h4>タイトル案</h4>{y.titles.map((title, i) => <label className="yt-copy-row" key={i}><span>{i + 1}</span><input aria-label={`YouTubeタイトル案${i + 1}`} maxLength={100} defaultValue={title} key={title} onChange={e => checkDraft({ titles: y.titles.map((t, n) => n === i ? e.target.value : t) })} onBlur={e => commitDraft({ titles: y.titles.map((t, n) => n === i ? e.target.value : t) }, e.currentTarget, title)}/><button title="タイトルをコピー" aria-label={`タイトル案${i + 1}をコピー`} disabled={draftInvalid || !title.trim()} onClick={() => void copy(data => data.titles[i] || '', `タイトル案${i + 1}`)}><Copy size={14}/></button></label>)}<label>概要欄<textarea aria-label="YouTube概要欄" rows={6} maxLength={3000} defaultValue={y.description} key={y.description} onChange={e => checkDraft({ description: e.target.value })} onBlur={e => commitDraft({ description: e.target.value }, e.currentTarget, y.description)}/></label><label>概要欄の末尾に付けるハッシュタグ（3個まで）<input aria-label="YouTubeハッシュタグ" placeholder="#動画編集 #字幕 #YouTube制作" defaultValue={(y.hashtags || []).join(' ')} key={(y.hashtags || []).join(' ')} onChange={e => checkDraft({ hashtags: parseHashtags(e.target.value) })} onBlur={e => commitDraft({ hashtags: parseHashtags(e.target.value) }, e.currentTarget, (y.hashtags || []).join(' '))}/></label><p className="yt-hint">空白またはカンマで区切ります。#は自動で補い、チャプターの後ろへ追加します。空欄にすると付けません。</p><label>検索ワード（10個・カンマ区切り）<input aria-label="YouTube検索ワード" defaultValue={y.keywords.join(',')} key={y.keywords.join(',')} onChange={e => checkDraft({ keywords: e.target.value.split(/[,，]/).map(k => k.trim()).filter(Boolean) })} onBlur={e => commitDraft({ keywords: e.target.value.split(/[,，]/).map(k => k.trim()).filter(Boolean) }, e.currentTarget, y.keywords.join(','))}/></label><span className="yt-hint">現在 {y.keywords.length}個</span></div><div><h4>チャプター</h4>{y.chapters.map((c, i) => <div className="yt-chapter" key={i}><input aria-label={`チャプター${i + 1}の秒`} type="number" min={0} step={1} defaultValue={c.time} key={c.time} onChange={e => checkDraft({ chapters: y.chapters.map((v, n) => n === i ? { ...v, time: seconds(e.target.value) } : v) })} onBlur={e => commitDraft({ chapters: y.chapters.map((v, n) => n === i ? { ...v, time: seconds(e.target.value) } : v) }, e.currentTarget, String(c.time))}/><input aria-label={`チャプター${i + 1}の見出し`} maxLength={100} defaultValue={c.label} key={c.label} onChange={e => checkDraft({ chapters: y.chapters.map((v, n) => n === i ? { ...v, label: e.target.value } : v) })} onBlur={e => commitDraft({ chapters: y.chapters.map((v, n) => n === i ? { ...v, label: e.target.value } : v) }, e.currentTarget, c.label)}/></div>)}<p className="yt-hint">00:00開始・3項目以上・各10秒以上。条件を満たすチャプターを概要欄のコピー時に付けます。</p>{!validChapters(previewChapters, duration) ? <p className="yt-notice">この長さ・時刻ではYouTubeチャプターの条件を満たしません。コピーには概要本文とハッシュタグを含めます。</p> : null}<label>コピーされる概要欄<textarea aria-label="チャプター・ハッシュタグ付きの概要欄" rows={6} readOnly value={descriptionWithChapters({ ...y, chapters: previewChapters }, duration)}/></label><button className="secondary-button" disabled={draftInvalid} onClick={() => void copy(descriptionWithChapters, '概要欄')}><Copy size={14}/>概要欄をチャプター付きでコピー</button></div></div> : <div className="yt-empty"><Youtube size={36}/><strong>字幕原稿をもとに、投稿素材をまとめます。</strong><p>まず「日本語字幕」で文字起こしを実行してください。</p></div>}<div className="yt-actions"><button className="secondary-button" disabled={!y.titles.length || draftInvalid} onClick={() => void copy(youtubeText, '投稿素材')}>投稿素材を一括コピー</button><button className="secondary-button" disabled={!y.titles.length || draftInvalid} onClick={() => void copy(data => data.keywords.join(','), '検索ワード')}>検索ワードをコピー</button><button className="secondary-button" disabled={!y.titles.length || draftInvalid} onClick={() => void save('txt')}>テキスト保存</button></div></> : null}
      {tab === 'thumbnail' ? <div className="yt-thumbnail">
        <div className="yt-thumbnail-controls"><span className="yt-thumbnail-eyebrow">動画からサムネイル</span><h3>動画の魅力が伝わる一枚に</h3>
          ...
          <div className="yt-thumbnail-reference" role="group" aria-label="サムネイルの参考画像">
          ...
          <button className="primary-button" disabled={!status?.configured || referenceMissing || (!y.thumbnailPrompt.trim()&&!p.clips.length&&!reference)} onClick={() => void task(async current => { const data=current.youtube||emptyYoutube(current);const asset = await window.luma!.aiThumbnail(current, data.thumbnailPrompt); accept(current, { ...data, thumbnailAssetId: asset.id }, [...current.assets, asset]); })}><Sparkles size={15}/>サムネイルを1枚生成</button>
          <p className="yt-hint">生成時に{reference ? '選んだ参考画像1枚、' : ''}動画から最大3枚の参考画像と、字幕・投稿文をOpenAIへ送信します。画像を選ぶだけでは送信しません。生成・再生成にはAPI利用料がかかります。完成後に人物の見た目・文字・内容を確認してください。</p>
        </div>
