...
import { ChevronLeft, ChevronRight, Expand, Pause, Play, SkipBack, SkipForward, Scan, Monitor, Camera, Hand } from 'lucide-react';
...
import { MONITOR_ZOOM_PRESETS } from '../preview-viewport';
...
export default function Preview({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const monitor = usePreviewViewport(stage, project, project.id), gestureActive = useEditor(s => s.gestureActive);
  const monitorLeft = (monitor.viewport.width - project.width * monitor.view.scale) / 2 + monitor.view.x;
  const monitorTop = (monitor.viewport.height - project.height * monitor.view.scale) / 2 + monitor.view.y;
  ...
  return <section className="preview-panel panel">
    <div className="panel-heading"><div className="panel-title"><Monitor size={15}/><span>プログラムモニター</span></div><div className="preview-transform-actions" ref={setTransformActions}/><span className="subtle tiny">{project.width} × {project.height} <span className="dot-separator">·</span> {project.fps} fps</span></div>
      <div className="media-elements" aria-hidden="true" ref={mediaBin}/><div className="preview-stage" ref={stage} tabIndex={-1} aria-label="モニターの表示領域" data-preview-zoom={monitor.view.scale} data-hand={monitor.hand} data-navigating={monitor.navigating} onPointerDownCapture={monitor.pointerDown} onPointerMoveCapture={monitor.pointerMove} onPointerUpCapture={monitor.pointerUp} onPointerCancel={monitor.pointerCancel} onLostPointerCapture={monitor.lostCapture} onAuxClick={event=>{if(event.button===1)event.preventDefault();}}>
      <div className="canvas-wrap preview-canvas" style={{ width:project.width*monitor.view.scale,height:project.height*monitor.view.scale,left:monitorLeft,top:monitorTop,'--preview-ratio':project.width/project.height,'--monitor-left':`${monitorLeft}px`,'--monitor-top':`${monitorTop}px`,'--monitor-width':`${monitor.viewport.width}px`,'--monitor-height':`${monitor.viewport.height}px` } as CSSProperties}><canvas ref={canvas} aria-label="動画プレビュー"/>{!readOnly ? <><MediaDragLayer sizes={mediaSizes} actions={transformActions} onSampleChroma={(clipId,point)=>sampleChroma.current(clipId,point)}/><TitleDragLayer fontVersion={fontVersion}/><DrawLayer/></> : null}{fontStatus ? <div className="preview-font-status" role="status">{fontStatus}<button className="text-button" onClick={() => setFontRetry(value => value + 1)}>再試行</button></div> : null}{safeGuides ? <div className="safe-guides"><div/></div> : null}{project.clips.length === 0 ? <div className="preview-empty"><Monitor size={36}/><strong>あなたの物語を、タイムラインへ。</strong><span>素材をドラッグして編集をはじめましょう</span></div> : null}</div>
      {project.width*monitor.view.scale>monitor.viewport.width-32||Math.abs(monitor.view.x)>.5?<input data-monitor-control className="monitor-pan horizontal" type="range" aria-label="モニターの横位置" title="モニターの表示位置を左右に移動" disabled={gestureActive} min={-monitor.limitX} max={monitor.limitX} step="any" value={-monitor.view.x} onChange={event=>monitor.pan('x',Number(event.target.value))}/>:null}
      {project.height*monitor.view.scale>monitor.viewport.height-32||Math.abs(monitor.view.y)>.5?<input data-monitor-control className="monitor-pan vertical" type="range" aria-label="モニターの縦位置" title="モニターの表示位置を上下に移動" disabled={gestureActive} min={-monitor.limitY} max={monitor.limitY} step="any" value={-monitor.view.y} onChange={event=>monitor.pan('y',Number(event.target.value))}/>:null}
      <div className="monitor-badge"><span/> PROGRAM</div>
    </div>
    <div className="preview-bottom"><div className="preview-meta"><span className="timecode accent">{timecode(playhead, project.fps)}</span><span className="shuttle-status" role="status" aria-label="シャトル状態">{playing ? `${shuttleRate < 0 ? '逆再生' : '再生'} ${Math.abs(shuttleRate)}×${audioLoading ? '・音声準備中' : ''}` : '停止'}</span><div className="preview-options"><select aria-label="プレビュー画質" value={quality} onChange={e => useEditor.setState({ previewQuality: Number(e.target.value) })}><option value={1}>フル画質</option><option value={0.5}>1/2 画質</option><option value={0.25}>1/4 画質</option></select><select aria-label="モニター表示倍率" title="表示倍率（ホイール・ピンチでも変更できます）" disabled={gestureActive} value={monitor.view.fit?'fit':String(monitor.view.scale)} onChange={event=>monitor.chooseScale(event.target.value)}><option value="fit">フィット</option>{MONITOR_ZOOM_PRESETS.map(scale=><option key={scale} value={scale}>{scale*100}%</option>)}{!monitor.view.fit&&!MONITOR_ZOOM_PRESETS.includes(monitor.view.scale)?<option value={monitor.view.scale}>{Number((monitor.view.scale*100).toFixed(1))}%</option>:null}</select><button className={`icon-button monitor-hand${monitor.hand?' active':''}`} aria-label="モニターの表示位置をドラッグで移動" aria-pressed={monitor.hand} title="表示位置をドラッグで移動（中ボタンでも操作できます）" disabled={gestureActive} onClick={()=>monitor.setHand(!monitor.hand)}><Hand size={15}/></button></div><span className="timecode subtle">{timecode(total, project.fps)}</span></div>
