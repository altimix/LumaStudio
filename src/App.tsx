...
import { ArrowUpRight, Check, CheckCircle2, ChevronDown, Download, FolderOpen, HelpCircle, Keyboard, LoaderCircle, Plus, Save, Settings2, Upload, X, RotateCcw, HardDrive, Monitor, Film, Volume2, ExternalLink } from 'lucide-react';
...
import { PREVIEW_LAYOUT, useLayoutPreferences } from './layout-preferences';
...
export default function App() {
  ...
  const [modal, setModal] = useState<'export' | 'shortcuts' | 'settings' | 'new' | 'youtube' | 'guide' | 'updates' | 'backups' | null>(null); const [fileMenu, setFileMenu] = useState(false); const [importLabel, setImportLabel] = useState(''); const [recovery, setRecovery] = useState<{ project: Project; savedAt: string } | null>(null); const [pending, setPending] = useState<(() => void) | null>(null);
  const [updateInfo, setUpdateInfo] = useState<UpdateInfo|null>(null);
  const [checkingUpdates, setCheckingUpdates] = useState(false);
  const checkUpdates = useCallback(async (refresh = false) => {
    if (!window.luma) return;
    setCheckingUpdates(true);
    try { setUpdateInfo(await window.luma.checkUpdates(refresh)); }
    catch { setUpdateInfo({status:'error',currentVersion:version,checkedAt:Date.now(),platform:'',arch:'',message:'更新情報を取得できませんでした。あとで再確認してください。'}); }
    finally { setCheckingUpdates(false); }
  }, []);
  useEffect(() => { if (ready) void checkUpdates(); }, [ready, checkUpdates]);
  ...
  const { preferences: layout, setPreferences: setLayout, resetLayout } = useLayoutPreferences();
  ...
  useEffect(() => useEditor.subscribe((state, previous) => {
    if (state.panel !== previous.panel || state.panelRequestId !== previous.panelRequestId) setLayout(current => current.libraryCollapsed ? { ...current, libraryCollapsed: false } : current);
  ...
  const collectMedia = async () => {
    setFileMenu(false); if (!window.luma || !beginProjectOperation('素材をまとめて保存しています')) return;
    try { const result = await window.luma.collectProject(useEditor.getState().project); if (result) useEditor.getState().notify(`素材付きプロジェクトを保存しました：${result}`); }
    catch (error) { useEditor.getState().notify(errorText(error)); } finally { endProjectOperation(); }
  };
  const relinkMediaFolder = async () => {
    setFileMenu(false); if (!window.luma || !beginProjectOperation('素材を一括で再リンクしています')) return;
    const snapshot = useEditor.getState().project;
    try {
      const result = await window.luma.relinkFolder(snapshot); if (!result || useEditor.getState().project !== snapshot) return;
      if (result.assets.length) useEditor.getState().commit({ ...snapshot, assets: snapshot.assets.map(asset => result.assets.find(fresh => fresh.id === asset.id) || asset) }, '素材を一括で再リンク');
      useEditor.getState().notify([`${result.assets.length}件を再リンクしました。`, ...result.unresolved].join('\n'));
    } catch (error) { useEditor.getState().notify(errorText(error)); } finally { endProjectOperation(); }
  };
  const showExport = () => { useEditor.getState().stop(); const project = useEditor.getState().project; setSettings({ width: project.width, height: project.height, fps: project.fps, quality: 'standard', format: 'mp4', encoder: 'auto' }); setPreset('match'); setProgress(null); setRenderError(''); setModal('export'); };
  const startExport = async () => {
    if (!window.luma) { setRenderError('書き出しはデスクトップアプリでご利用ください。'); return; }
    const attempt={native:false};exportAttempt.current=attempt;setPreparingFonts(settings.format !== 'mp3');
    setRendering(true); setProgress(null); setRenderError('');
    try {
      const project = useEditor.getState().project;
      if (settings.format !== 'mp3') {
        await ensureProjectFonts(project);
        if (exportAttempt.current !== attempt) return;
        await document.fonts.ready;
        if (exportAttempt.current !== attempt) return;
      }
      setPreparingFonts(false);
      if (settings.format !== 'mp3' && settings.target === 'shorts' && (project.width * 16 !== project.height * 9 || endTime(project) > 180 + 0.000001)) throw new Error('Shortsは縦型9:16のシーケンス・3分以内で書き出せます。シーケンス設定と長さを確認してください。');
      attempt.native = true;
      const result = await window.luma.exportProject(project, settings, settings.format === 'mp3' ? {} : renderTitles(project));
  ...
  return <div className="app" onKeyDownCapture={e => { if (projectBusyRef.current && !(e.target instanceof HTMLElement && e.target.closest('.project-operation-dialog'))) { e.preventDefault(); e.stopPropagation(); } }} onDragOver={e => { if (e.dataTransfer.types.includes('Files')) e.preventDefault(); }} onDrop={e => { if (e.dataTransfer.files.length) { e.preventDefault(); if (savingOnClose || projectBusyRef.current) return; if (window.luma) void importMedia(Array.from(e.dataTransfer.files)); else void browserImport(e.dataTransfer.files); } }}>
    <div className="app-titlebar"><div className="brand"><svg width="23" height="25" viewBox="0 0 23 25" aria-hidden="true"><path d="M3 2h6v15h11v6H3z" fill="#b9d993"/><path d="m12 3 8 8h-8z" fill="#778e61"/></svg><span>Luma<span>Studio</span></span></div><div className="app-menu"><div className="file-menu-wrap"><button onClick={() => { setWindowMenu(false); setFileMenu(!fileMenu); }}>ファイル</button>{fileMenu ? <><button className="menu-dismiss" aria-label="メニューを閉じる" onClick={() => setFileMenu(false)}/><div className="popup-menu file-menu"><button onClick={() => safely(newProject)}><Plus size={14}/>新規プロジェクト<kbd>{shortcutLabel('Ctrl N')}</kbd></button><button onClick={() => safely(() => { void open(); })}><FolderOpen size={14}/>プロジェクトを開く<kbd>{shortcutLabel('Ctrl O')}</kbd></button><hr/><button onClick={() => { void save(); setFileMenu(false); }}><Save size={14}/>保存<kbd>{shortcutLabel('Ctrl S')}</kbd></button><button onClick={() => { void save(true); setFileMenu(false); }}><Save size={14}/>名前を付けて保存</button><hr/><button onClick={() => { void importMedia(); setFileMenu(false); }}><Upload size={14}/>素材を読み込む<kbd>{shortcutLabel('Ctrl I')}</kbd></button><button disabled={!isDesktop} onClick={() => { useEditor.getState().stop(); setFileMenu(false); setModal('backups'); }}>バックアップ履歴</button><button disabled={!window.luma} onClick={() => void collectMedia()}>素材をまとめて保存</button><button disabled={!window.luma || !p.assets.some(asset => asset.offline)} onClick={() => void relinkMediaFolder()}>フォルダから一括再リンク</button><button onClick={() => { showExport(); setFileMenu(false); }}><Download size={14}/>動画を書き出す<kbd>{shortcutLabel('Ctrl E')}</kbd></button></div></> : null}</div><EditMenu onOpen={() => { setFileMenu(false); setWindowMenu(false); }}/><button onClick={editSettings}>シーケンス</button><div className="file-menu-wrap"><button aria-expanded={windowMenu} onClick={() => { setFileMenu(false); setWindowMenu(!windowMenu); }}>ウィンドウ</button>{windowMenu ? <><button className="menu-dismiss" aria-label="ウィンドウメニューを閉じる" onClick={() => setWindowMenu(false)}/><div className="popup-menu file-menu"><button aria-pressed={historyOpen} onClick={() => { setHistoryOpen(!historyOpen); setWindowMenu(false); }}>ヒストリー{historyOpen ? <Check size={14}/> : null}</button><hr/><button aria-pressed={!layout.libraryCollapsed} onClick={() => { setLayout(current => ({ ...current, libraryCollapsed: !current.libraryCollapsed })); setWindowMenu(false); }}>素材パネル{!layout.libraryCollapsed ? <Check size={14}/> : null}</button><button aria-pressed={!layout.inspectorCollapsed} onClick={() => { setLayout(current => ({ ...current, inspectorCollapsed: !current.inspectorCollapsed })); setWindowMenu(false); }}>プロパティパネル{!layout.inspectorCollapsed ? <Check size={14}/> : null}</button><hr/><button onClick={() => { setLayout(current => ({ ...current, ...PREVIEW_LAYOUT })); setWindowMenu(false); }}>プレビュー優先の配置</button><button onClick={() => { resetLayout(); setWindowMenu(false); }}><RotateCcw size={14}/>レイアウトを初期状態に戻す</button></div></> : null}</div><HelpMenu onSelect={page => {setFileMenu(false);setWindowMenu(false);useEditor.getState().stop();setModal(page);}} onClearCache={() => { void window.luma?.clearExportMaskCache().then(({ remainingBytes, inUse }) => useEditor.getState().notify(inUse || remainingBytes ? "使用中のマスクキャッシュを除き、削除しました。" : "書き出し用マスクキャッシュを削除しました。")).catch(error => useEditor.getState().notify(errorText(error))); }}/></div><div className="titlebar-center">{p.name}{dirty ? <span className="unsaved-dot"/> : null}</div><span className="titlebar-version">{updateInfo?.status === 'available' ? <button className="update-badge" onClick={() => setModal('updates')}>更新あり</button> : null}{isDesktop ? version : 'BROWSER PREVIEW'}</span></div>
    ...
    {modal === 'updates' ? <Modal title="アップデート" onClose={() => setModal(null)}><UpdateHelp info={updateInfo} checking={checkingUpdates} onCheck={() => {void checkUpdates(true);}}/></Modal> : null}
    {modal === 'backups' ? <BackupHistory onClose={() => setModal(null)} onRestore={snapshot => { setModal(null); safely(() => { setRecovery(null); useEditor.getState().load(snapshot); useEditor.setState({ dirty: true }); void window.luma?.resetProjectPath(true).catch(e => useEditor.getState().notify(errorText(e))); }); }}/> : null}
    {modal === 'shortcuts' ? <ShortcutHelp onClose={() => setModal(null)}/> : null}
    {historyOpen ? <HistoryPanel onClose={() => setHistoryOpen(false)}/> : null}
    {modal === 'settings' ? <Modal title="シーケンス設定" onClose={() => setModal(null)}><div className="settings-form"><label>プロジェクト名<input value={settingsDraft.name} maxLength={120} onChange={e => setSettingsDraft({ ...settingsDraft, name: e.target.value })}/></label><label>フレームサイズ<select value={`${settingsDraft.width}x${settingsDraft.height}`} onChange={e => { const [width,height] = e.target.value.split('x').map(Number); setSettingsDraft({ ...settingsDraft, width, height }); }}><option value="1920x1080">YouTube 横動画 — 1920 × 1080 (16:9)</option><option value="3840x2160">4K UHD — 3840 × 2160 (16:9)</option><option value="1280x720">HD — 1280 × 720 (16:9)</option><option value="1080x1920">YouTube Shorts — 1080 × 1920 (9:16)</option><option value="1080x1080">スクエア — 1080 × 1080 (1:1)</option></select></label><label>フレームレート<select aria-label="シーケンスのフレームレート" value={settingsDraft.fps} onChange={e => setSettingsDraft({ ...settingsDraft, fps: Number(e.target.value) })}>{[...new Set([p.fps,24,25,30,50,60])].sort((a,b)=>a-b).map(fps => <option key={fps} value={fps}>{fps} fps</option>)}</select></label></div>{settingsError ? <div className="export-error" role="alert">{settingsError}</div> : null}<div className="modal-footer"><button className="secondary-button" onClick={() => setModal(null)}>キャンセル</button><button className="primary-button" onClick={saveSettings}>設定を適用</button></div></Modal> : null}
    {modal === 'guide' ? <Modal title="はじめての動画編集" onClose={() => setModal(null)}><EditingGuide hasClips={p.clips.length > 0} onImport={() => { setModal(null); void importMedia(); }} onTitles={() => { setModal(null); revealPanels(); useEditor.setState({ panel:'titles', inspectorTab:'video' }); }} onExport={showExport} onNew={() => { setModal(null); safely(newProject); }} onShortcuts={() => setModal('shortcuts')}/></Modal> : null}
    {modal === 'export' ? <Modal title={settings.format === 'mp3' ? '音声をMP3で書き出す' : '動画を書き出す'} onClose={() => { if (!rendering) setModal(null); }}><div className="export-intro"><span className="export-icon">{settings.format === 'mp3' ? <Volume2 size={24}/> : <Film size={24}/>}</span><div><h3>{p.name}</h3><p>{settings.format === 'mp3' ? 'MP3 音声のみ' : 'H.264 / MP4 · AAC ステレオ'} <span>·</span> {timecode(endTime(p),p.fps)}</p></div></div>
      {progress?.status === 'complete' ? <div className="export-success"><CheckCircle2 size={46}/><h3>書き出しが完了しました</h3><p>{progress.output}</p>{progress.encoderLabel ? <p className="export-encoder-used">使用した方式：{progress.encoderLabel}</p> : null}{progress.warning ? <p className="export-fallback-note">GPUを使用できなかったため、CPUで書き出しました。</p> : null}<button className="primary-button" onClick={() => { void window.luma?.reveal(progress.output); }}><FolderOpen size={16}/>保存先を開く</button></div> : <><fieldset className="settings-form" disabled={rendering}><label>書き出し形式<select aria-label="書き出し形式" value={settings.format || 'mp4'} onChange={e => setSettings({ ...settings, format: e.target.value as ExportSettings['format'] })}><option value="mp4">動画（MP4）</option><option value="mp3">音声のみ（MP3）</option></select></label>{settings.format !== 'mp3' ? <><label>書き出しサイズ<select aria-label="書き出しサイズ" value={preset} onChange={e => { const target = e.target.value; setPreset(target); if (target === 'youtube' || target === 'shorts') { setSettings({ ...settings, width: target === 'shorts' ? 1080 : 1920, height: target === 'shorts' ? 1920 : 1080, target }); return; } const portrait = p.height > p.width; const square = p.height === p.width; const [w,h] = e.target.value === 'match' ? [p.width,p.height] : e.target.value === '720' ? [1280,720] : e.target.value === '4k' ? [3840,2160] : [1920,1080]; setSettings({ ...settings, target: undefined, width: e.target.value === 'match' ? w : square ? h : portrait ? h : w, height: e.target.value === 'match' ? h : square ? h : portrait ? w : h }); }}><option value="match">シーケンスに一致 — {p.width} × {p.height}</option><option value="youtube">YouTube 横動画 — 1920 × 1080</option><option value="shorts">YouTube Shorts — 1080 × 1920・3分以内</option><option value="1080">1080p / フルHD</option><option value="720">720p / HD</option><option value="4k">2160p / 4K UHD</option></select></label><div className="form-columns"><label>フレームレート<select aria-label="書き出しフレームレート" value={settings.fps} onChange={e => setSettings({ ...settings, fps: Number(e.target.value) })}>{[...new Set([p.fps,24,25,30,50,60])].sort((a,b)=>a-b).map(fps => <option key={fps} value={fps}>{fps} fps</option>)}</select></label><label>品質<select aria-label="品質" value={settings.quality} onChange={e => setSettings({ ...settings, quality: e.target.value as ExportSettings['quality'] })}><option value="standard">標準 — バランス</option><option value="high">高画質 — 細部を優先</option><option value="draft">ドラフト — 高速</option></select></label></div><ExportEncoder value={settings.encoder || 'auto'} onChange={encoder => setSettings({ ...settings, encoder })}/></> : <p className="field-help">タイムラインの音声をミックスしてMP3で保存します。映像は書き出しません。</p>}</fieldset><div className="export-specs">{settings.format === 'mp3' ? <span>MP3 音声 <strong>48 kHz / ステレオ / 192 kbps</strong></span> : <><span>映像サイズ <strong>{settings.width} × {settings.height}</strong></span><span>オーディオ <strong>48 kHz / 192 kbps（再圧縮時）</strong></span></>}</div>
      {rendering ? <div className="export-progress" role="status"><div><span><LoaderCircle className="spin" size={15}/>{preparingFonts?'日本語フォントを準備しています…':progress?.status === 'preparing' ? (settings.format === 'mp3' ? '音声を準備しています…' : '映像・音声と書き出し方式を準備しています…') : progress ? (settings.format === 'mp3' ? 'MP3を書き出しています…' : '映像を書き出しています…') : '保存先と素材を準備しています…'}</span><strong>{Math.round((progress?.progress || 0) * 100)}%</strong></div><progress value={progress?.progress || 0} max="1"/><small>{progress?.encoderLabel || (settings.format === 'mp3' ? 'タイムラインの音声をMP3にまとめています。' : '編集内容と音声を1本の動画にまとめています。')}</small>{progress?.warning ? <p className="export-fallback-note">{progress.warning}</p> : null}</div> : null}
      {renderError ? <div className="export-error" role="alert">{renderError}</div> : null}<div className="modal-footer"><span className="export-local"><HardDrive size={13}/>このPCで処理</span>{rendering ? <button className="secondary-button" onClick={() => { if(exportAttempt.current?.native){void window.luma?.cancelExport();}else{exportAttempt.current=null;setRendering(false);setPreparingFonts(false);setRenderError('書き出しをキャンセルしました。');} }}>書き出しを中止</button> : <button className="primary-button" disabled={!p.clips.length} onClick={() => { void startExport(); }}><Download size={16}/>保存先を選んで{settings.format === 'mp3' ? 'MP3を' : ''}書き出す</button>}</div></>}
    </Modal> : null}
